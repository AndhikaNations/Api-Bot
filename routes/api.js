...
const listkey = ["Abxbot", "AndhikaGanz"];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AndhikaGanteng'+'ANDHIKA--GANS';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Abxbot') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Abxbot')  return res.json(loghandler.invalidKey)
